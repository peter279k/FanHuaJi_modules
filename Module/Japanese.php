...
class Japanese implements ModuleInterface {
    ...
    // my own defined text
    private $extraKanji2  = '坤蝉澤剥碍逢繋尨倶冨巌廼弍彦掴灯焔罸聡蒋遥騨鬪凛撫云瀾涸子丑寅卯辰巳午未申酉戌亥劫芦鮎馴懺棲栖霞祐卐卍勝'; // added by myself
    private $kanjiOnlyJpn = '歩両乗亀亜仏仮伝価倹児処剣剤剰労効勅勧勲単厳収営団囲図圧塀塁壊壱売変奨実寛対専巣帯帰庁広廃弐弾従徳応恵悩悪懐戦戻払抜択拝拠拡挙挿捜掲揺摂斉斎暁暦曽栄桜桟検楽様権歓歯歴毎気浄涙渇済渉渋渓満滝焼猟獣畳発県砕稲穂穏竜粋粛経絵継続総緑縁縄縦繊聴脳荘蔵薬蛍覚観訳読謡豊賛転軽辺逓遅郷酔釈鉄鉱銭鋳錬録関闘陥険隠雑霊頼顕駆騒験髄髪鶏黒黙齢聡凛氷衆浜巌鉢鮎歳';
    ...
    private $fixesJpnWord = [
        ...
        '([\d０１２３４５６７８９先今来次隔毎]|黄金)周' => '$1週',
        '([の])周(?![期波速長回濠囲王朝]|[り]|.?期|回路)' => '$1週',
        '([皇蟲虫])後' => '$1后',
        '後([位腹妃羿]|がね)' => '后$1',
        '([北])鬪' => '$1斗',
        '鬪([形米山南酒拱掻])' => '斗$1',
    ...
    public function __construct ($options=null) {
        ...
        $fixesChiWord_uniqueChars = $this->array_unique_fast(
            ...
            preg_split(
                '//u',
                implode('', array_keys($this->fixesChiWord)),
                -1,
                PREG_SPLIT_NO_EMPTY
            )
        );
        $fixesJpnCharChiWord_keyval = implode('',
            array_merge(
                array_keys($this->fixesJpnChar),
                array_values($this->fixesJpnChar),
                $fixesChiWord_uniqueChars
            )
        );
        $jpnChars      = "{$gojuon}{$toyokanji}{$extraKanji}{$extraKanji2}{$punctuations}{$numbers}";
        $maybeJpnChars = "{$jpnChars}{$fixesJpnCharChiWord_keyval}";
        $onlyJpn       = "{$gojuon}{$kanjiOnlyJpn}";
        // multi-byte strings
        $this->mbOnlyJpn       = new MbString($onlyJpn, $encoding);
        $this->mbMaybeJpnChars = new MbString($maybeJpnChars, $encoding);
    }

    // destructor
    public function __destruct () {
        unset($this->mbOnlyJpn, $this->mbMaybeJpnChars);
    }
    ...
    private function searchConsecutiveJpnChars (MbString &$mbText, $offset=0) {
        ...
        for ($idx=$offset; $idx<$mbText_len; ++$idx) {
            ...
            if (!$this->mbOnlyJpn->has($mbText[$idx])) continue;
